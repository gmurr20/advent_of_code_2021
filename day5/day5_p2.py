...
def print_example(point_to_line_count):
    for y in range(10):
        curr_line = ""
        for x in range(10):
            if not (x,y) in point_to_line_count:
                curr_line += '.'
            else:
                curr_line += str(point_to_line_count[(x,y)])
        print(curr_line)
...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        assert(slope == -1.0 or slope == 1.0)
        slope = int(slope)
        curr_x = starting_point[0]
        curr_y = starting_point[1]
        for i in range(abs(starting_point[0] - ending_point[0]) + 1):
            pos = (curr_x,curr_y)
            if not pos in point_to_line_count:
                point_to_line_count[pos] = 0
            point_to_line_count[pos] += 1
